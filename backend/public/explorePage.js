  ...
  const map = new maptilersdk.Map({
    container: "map",
    style: `https://api.maptiler.com/maps/streets/style.json?key=P2YnxuVUfPXTTg1SJKHA`,
  ...
  async function loadDestinations() {
    const res = await fetch(`${API}/explore/filter/a`);
    const data = await res.json();
    const destinations = JSON.parse(data.destinations);
    destinations.forEach((dest) => {
      // Add marker
      new maptilersdk.Marker()
        .setLngLat([dest.lng, dest.lat]) // IMPORTANT: [lng, lat]
        .addTo(map);
    });
 ...
 async function getDestinations(id) {
    ...
    destinationSection.innerHTML = destinationsArray
      .map(
        (el) => `
     <article id=${el.id} class="main__top-destination">
  ...
  document
    .querySelector(".main__destinations")
    .addEventListener("click", async (e) => {
      if (e.target.parentElement.matches(".main__top-destination")) {
        // const destinationEles = Array.from(nodes);
        // console.log(destinationEles);
        // destinationEles.forEach((destinationEle) => {
        // destinationEle.addEventListener("click", async function (evt) {
        //console.log(evt.currentTarget.id);
        const fetchedData = await fetch(
          `${API}/destinations/${e.target.parentElement.id}`,
        );
        const body = await fetchedData.json();
     ...
     <header style="background-image:url(destinations/${destination.image})" class="destinationDialog__header destinationDialog__header-background-img">
             <div class="destinationDialog__header-menu">
               ...
               <button class="closeModal" onclick="(() =>  {document.body.style.overflow = '';})()">
         ...
         <main class="destinationDialog__main">
             ...
             <section class="destinationDialog__main-tabs">
               <ul class="destinationDialog__main-tabs-btns">
                 <li class="destinationDialog__main-tabs-item"><button class="destinationDialog__main-tabs-btn" onclick="(() => {document.querySelector('.tabOverview').classList.add('active');document.querySelector('.tabItinerary').classList.remove('active');document.querySelector('.howToReach').classList.remove('active');})()">Overview</button></li>
                 <li class="destinationDialog__main-tabs-item"><button class="destinationDialog__main-tabs-btn" onclick="(() => {document.querySelector('.tabItinerary').classList.add('active');document.querySelector('.tabOverview').classList.remove('active');document.querySelector('.howToReach').classList.remove('active');})()">Itineray Ideas</button></li>
                 <li class="destinationDialog__main-tabs-item"><button class="destinationDialog__main-tabs-btn" onclick="(() => {document.querySelector('.howToReach').classList.add('active');document.querySelector('.tabOverview').classList.remove('active');document.querySelector('.tabItinerary').classList.remove('active');})()">How to reach</button></li>
               </ul>
                 ...
                 <p class="destinationDialog__main-tabs-topattractions-head">Top Attraction</p>
                   ...
                   ${destination.topAttractions
                     .map(
                       (
                         el,
                       ) => `<li class="destinationDialog__main-tabs-topattraction">
               ...
               <section class="dialog-overviewinfo">
                 <div class="overview-quickinfo">
                   ...
                   <p><span>Starting from</span> <span style="color:#42f54e;font-size:large">${getFormattedAmout(destination.startingPrice)}<span style="font-size:small;color:rgba(255,255,255,0.8)">/person</span></span>  </p>
     ...
     `;
        ...
        Array.from(topImgs).forEach((el) => {
          fetch(
            "https://api.unsplash.com/photos/random?query=travel&client_id=uEKuqX6Pyj60Sp7kG8ShXlLzsXBK7GXBTF20c4TjL_c",
          )
            .then((res) => res.json())
            .then((data) => {
      ...
      const overViewBtn = document.querySelector(
        ".destinationDialog__main-tabs-btn",
      );
      overViewBtn.focus();
      document.querySelector(".tabOverview").classList.add("active");
    });

  // const explorePage = document.getElementById("explore-link");
  // explorePage.classList.add("active-link");

  const allBtns = document.querySelectorAll(".main__filter-btn");

  Array.from(allBtns).forEach((el) => {
    el.addEventListener("click", (evt) => {
      allBtns.forEach((el2) => el2.classList.remove("btn-active"));
      // const all_dest_btn = document.getElementById("all-destinations__filter");
  ...
  const main_destinations = document.querySelector(".main__destinations");
  const searchInput = document.querySelector(".main__filter-search");
  let timeout;
  searchInput.addEventListener("input", (evt) => {
    ...
    timeout = setTimeout(async () => {
      ...
      if (!query) {
        ...
        para.style = "text-align:center;color:white";
        main_destinations.innerHTML = "";
        main_destinations.appendChild(para);
        return;
      }
      main_destinations.innerHTML = "Loading...";
      const res = await fetch(
        `${API}/explore/search?q=${query}`,
      );
      const data = await res.json();
      console.log(data);
      if (data.length === 0) {
      ...
      main_destinations.innerHTML = data
        .map(
          (el) => `<article id=${el.id} class="main__top-destination">
           <p class="main__destination-rating-package">${el.rating}💠${el.totalPackages} packages</p>
           <section class="top-destination__img" style="background-image:url('/destinations/${el.image}')"></section>
           <section class="top-destination__about">
             <p>${el.name}, ${el.state}</p>
...
const el = document.querySelector('.nav__item');
